...
const TX_META = {
  sale:       { label: 'Satış',        color: 'bg-[#82e05a]/15 text-[#5da83f] border border-[#82e05a]/30', icon: ShoppingCart,  isPositive: true },
  purchase:   { label: 'Alış',         color: 'bg-rose-500/10 text-rose-600 border border-rose-500/20',    icon: ArrowUpRight,  isPositive: true },
  payment:    { label: 'Tahsilat',     color: 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20', icon: ArrowDownLeft, isPositive: false },
  return:     { label: 'İade',         color: 'bg-orange-500/10 text-orange-600 border border-orange-500/20',   icon: RotateCcw,    isPositive: false },
  adjustment: { label: 'Düzeltme',    color: 'bg-slate-500/10 text-slate-600 border border-slate-500/20',     icon: CopyMinus,    isPositive: false },
  debt:       { label: 'Veresiye',    color: 'bg-indigo-500/10 text-indigo-600 border border-indigo-500/20',  icon: FileText,     isPositive: true },
  refund:     { label: 'Nakit İade',  color: 'bg-orange-500/10 text-orange-600 border border-orange-500/20',  icon: ArrowUpLeft,  isPositive: false },
    ...
    <div className="pt-1 pb-20 sm:pb-10 relative">
      ...
      <div className="flex items-center justify-between mb-4 gap-2">
        ...
        <div className="relative flex-1 sm:flex-none">
          ...
          <input
            type="text"
            placeholder="Fiş no veya açıklama..."
            value={searchTerm}
        ...
        <table className="w-full text-sm divide-y divide-slate-200">
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            ...
            {paginated.map((tx, i) => {
              ...
              return (
                <tr key={tx.id || i}
                  onClick={isSale && tx.reference_id ? () => navigate(`/sales/${tx.reference_id}`) : undefined}
                  className={`transition-colors ${isSale && tx.reference_id ? 'hover:bg-[#82e05a]/5 cursor-pointer' : 'hover:bg-gray-50/80'}`}>
                  ...
                  <td className="px-4 py-[11.5px] whitespace-nowrap text-xs font-semibold tabular-nums">
                    {isSale && (tx.sale_number || tx.reference_id) ? (
                      <span className="font-mono bg-[#82e05a]/10 text-[#5da83f] px-2 py-0.5 rounded border border-[#82e05a]/25 flex items-center gap-1 w-fit">
                        <FileText className="w-3 h-3" />{tx.sale_number || `#${tx.reference_id}`}
                      </span>
                    ) : '—'}
                  </td>
                  <td className="px-4 py-[11.5px] text-xs text-gray-600 font-medium truncate max-w-[200px]" title={tx.notes || '—'}>{tx.notes || '—'}</td>
                  <td className="px-4 py-[11.5px] whitespace-nowrap text-xs font-medium text-gray-500">{fmtDate(tx.created_at)}</td>
                  <td className="px-4 py-[11.5px] font-bold tabular-nums whitespace-nowrap"><span className="text-slate-800">{fmt(Math.abs(tx.amount))}</span></td>
          ...
          paginated.map((tx, i) => {
            ...
            return (
              <div
                key={tx.id || i}
                onClick={isSale && tx.reference_id ? () => navigate(`/sales/${tx.reference_id}`) : undefined}
                className={`bg-white rounded-xl border border-slate-200 overflow-hidden ${isSale && tx.reference_id ? 'active:scale-[0.99] cursor-pointer hover:border-[#82e05a]/50 hover:shadow-sm' : ''} transition-all`}
              >
