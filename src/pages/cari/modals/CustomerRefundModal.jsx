...
export const CustomerRefundModal = ({ isOpen, onClose, customer, onSaved }) => {
  const receivable = Math.abs(parseFloat(customer?.balance) || 0);
  const today = new Date().toISOString().split('T')[0];

  const [type, setType] = useState('cash_refund'); // 'cash_refund' | 'offset'
  ...
  const afterBalance = -receivable + parsedAmount; // customer balance goes from negative toward 0
  const isAmountValid = parsedAmount > 0 && parsedAmount <= receivable;

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isAmountValid) {
      toast.error('Geçerli bir iade tutarı girin. Maksimum: ' + fmt(receivable));
      return;
    }
    setLoading(true);
    try {
      await customerService.refundCustomer(
        customer.id,
        parsedAmount,
        type,
        method,
        type === 'cash_refund' ? parseInt(registerId) : null,
        date,
        description
      );
      const label = type === 'cash_refund' ? 'Nakit İade' : 'Mahsuplaşma';
      toast.success(`${fmt(parsedAmount)} ${label} kaydedildi.`);
      onSaved();
      onClose();
    } catch (err) {
      console.error('[CustomerRefund] İade Hatası:', err);
      toast.error(err?.message || 'İade/Mahsup işlemi başarısız.');
    } finally {
      setLoading(false);
    }
  };

  const inputCls = 'w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-400/50 focus:border-orange-400 transition-all';
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-orange-50 border border-orange-100">
              <ArrowUpLeft className="w-4 h-4 text-orange-500" />
            </div>
            <div>
              <h2 className="text-base font-bold text-slate-900">Müşteriye İade / Mahsup</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-xl px-4 py-3 text-center">
              <p className="text-[10px] uppercase font-bold tracking-widest text-orange-500 mb-1">Müşteri Alacağı</p>
              <div className="text-2xl font-black text-orange-600 tracking-tight">{fmt(receivable)}</div>
            </div>
              ...
              {[
                {
                  id: 'cash_refund',
                  title: 'Nakit İade',
                  desc: 'Kasadan çıkış yapılır.',
                  icon: Banknote,
                  activeStyle: { background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.35)', boxShadow: '0 2px 8px rgba(249,115,22,0.1)' },
                  inactiveStyle: { background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(209,213,219,0.6)' },
                },
                {
                  id: 'offset',
                  title: 'Mahsuplaşma',
                  desc: 'Sonraki alışından düşünülür.',
                  icon: RefreshCw,
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <Icon className={`w-4 h-4 ${isActive ? (opt.id === 'cash_refund' ? 'text-orange-600' : 'text-blue-600') : 'text-slate-400'}`} />
                      <span className={`text-xs font-bold ${isActive ? (opt.id === 'cash_refund' ? 'text-orange-700' : 'text-blue-700') : 'text-slate-600'}`}>{opt.title}</span>
                    </div>
            ...
            {/* Tutar */}
            <div>
              <label className={labelCls}>İade / Mahsup Tutarı (₺) <span className="text-rose-500">*</span></label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`${inputCls} pl-7 font-bold tabular-nums text-lg text-orange-600`}
                  placeholder="0.00"
                />
              </div>
            ...
            {/* Nakit İade ek alanlar */}
            {type === 'cash_refund' && (
              <>
                {/* Yöntem Seçici */}
                <div>
                  <label className={labelCls}>İade Yöntemi</label>
                ...
                {(method === 'cash' || method === 'credit_card') && (
                  <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                    <label className={labelCls}>Hangi Kasadan</label>
                    <div className="relative">
                ...
                {/* Tarih */}
                <div>
                  <label className={labelCls}>İade Tarihi</label>
            ...
            {type === 'offset' && (
              <>
                ...
                <div className="flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-lg p-3">
                  ...
                  <p className="text-xs text-blue-600">Mahsuplaşma kasa bakiyenizi etkilemez. Müşterinin sonraki alışında bu tutar bakiyesinden düşülür.</p>
                </div>
              </>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={type === 'offset' ? 'Sonraki alışta düşülecek, vb.' : 'Fazla ödeme iadesi, vb.'}
              />
            </div>
            ...
            {parsedAmount > 0 && (
              <div className="bg-slate-50 border border-slate-100 rounded-lg p-3 space-y-1.5">
                <div className="flex justify-between text-xs text-slate-500">
                  ...
                  <span className="font-semibold text-orange-600">{fmt(receivable)}</span>
                </div>
                <div className="flex justify-between text-xs text-slate-500">
                  <span>{type === 'offset' ? 'Mahsuplaşma' : 'İade Tutarı'}</span>
                ...
                <div className="flex justify-between text-xs border-t border-slate-200 pt-1.5">
                  ...
                  <span className={`font-black ${afterBalance < -0.001 ? 'text-orange-600' : afterBalance > 0.001 ? 'text-rose-600' : 'text-slate-500'}`}>
                    {fmt(Math.abs(afterBalance))}
                    {afterBalance < -0.001 ? ' (Alacaklı)' : afterBalance > 0.001 ? ' (Borçlu)' : ' (Dengede)'}
                  </span>
                </div>
              </div>
            )}
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm active:scale-95 transition-all focus:outline-none disabled:opacity-60 disabled:cursor-not-allowed ${
                type === 'offset'
                  ...
                  : 'text-orange-700 bg-orange-50 border border-orange-200 hover:bg-orange-100 focus:ring-2 focus:ring-orange-400 focus:ring-offset-2'
              }`}
            >
              {type === 'offset' ? <RefreshCw className="w-4 h-4" /> : <ArrowUpLeft className="w-4 h-4" />}
              {loading ? 'Kaydediliyor...' : type === 'offset' ? 'Mahsuplaşmayı Kaydet' : 'İadeyi Kaydet'}
            </button>
          </div>
        </form>
      </div>
