      ...
      pages.push(
        <button key={i} onClick={() => setCurrentPage(i)}
          className={`w-7 h-7 text-xs rounded-lg flex items-center justify-center transition-all ${
            currentPage === i ? 'bg-[#7ed957]/20 text-[#5da83f] border border-[#7ed957]/40 font-bold' : 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50'
          }`}
        >{i}</button>
      );
    }
            ...
            <div className="flex flex-col flex-1 overflow-hidden p-6">
              ...
              {filtered.length > 0 && (
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200 flex-none px-2">
                  <div className="text-sm text-slate-500 font-bold">{filtered.length} ürün — Sayfa {currentPage}/{totalPages}</div>
