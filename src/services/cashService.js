  ...
  async creditCardPayment(creditRegId, sourceRegId, amount, notes, transactionDate) {
    try {
      ...
      if (isSupabase()) {
        ...
        await supabase.from('cash_transactions').insert([{ register_id: creditRegId, transaction_type: 'credit_payment_in', amount, balance_after: creditNewBal, notes: `Ödeme Alındı: ${sourceReg.name}${notes ? ' - ' + notes : ''}`, created_at: dateVal + 1 }]);
        return true;
      }
      ...
      return await db.transaction('rw', db.cash_registers, db.cash_transactions, async () => {
        ...
        await db.cash_transactions.add({ register_id: Number(creditRegId), transaction_type: 'credit_payment_in', amount, balance_after: creditNewBal, notes: `Ödeme Alındı: ${sourceReg.name}${notes ? ' - ' + notes : ''}`, created_at: dateVal + 1 });
        return true;
      });
